...
# @frappe.whitelist()
# def get_import_items(last_req_dt: str = "20160523000000", dcl_ref_num: Optional[str] = None) -> Dict[str, Any]:
#     if not is_zra_enabled():
...
#         return make_vsdc_request("imports/selectImportItems", payload)

#     except Exception as e:
#         frappe.log_error(title="ZRA API Error: Get Imports", message=frappe.get_traceback())
#         frappe.throw(f"Failed to retrieve import items from ZRA: {str(e)}")
...
def update_import_items(
    ...
    """
    Acknowledge or disregard imported items received from Smart Invoice.
    This acts strictly as the external sync layer.
    """
    # if not is_zra_enabled():
    #     frappe.throw("ZRA integration is not configured for this site.")
...
# @frappe.whitelist()
# def process_imported_declarations(payload: Union[str, Dict[str, Any]]) -> Dict[str, Any]:
#     """
#     Main Controller: Orchestrates the workflow and handles the transaction.
#     """
#     try:
...
#         target_warehouse = data.get("target_warehouse") or _get_default_warehouse()
...
#         _create_stock_entry(items, target_warehouse)
...
#         task_cd_log = payload.get("taskCd", "Unknown") if isinstance(payload, dict) else "Unknown"
#         frappe.log_error(frappe.get_traceback(), f"Import Processing Failed (Task {task_cd_log})")
...
#     """
#     Translates the submitted Stock Entry into ZRA's stock update format
#     and sends it via the VSDC client.
#     If this fails, the exception cascades up and rolls back the ERP Stock Entry.
#     """
#     if not is_zra_enabled():
#         return
...
#     # Construct your specific ZRA stock payload here.
#     # (Update endpoints/keys based on your exact ZRA inventory documentation)
#     zra_stock_payload = {
#         "tpin": config.get("tpin"),
#         "bhfId": config.get("bhf_id"),
#         "regrNm": _zra_user_id(),
...
#             "rsnCd": "04", # Adjust Reason Code for Imports/Purchases
#             "inQty": item.qty,
#             # Add other necessary ZRA inventory parameters
#         })

#     # Execute the request (will raise an exception if it fails, triggering rollback)
#     make_vsdc_request("inventory/saveStockIO", zra_stock_payload) # Use correct endpoint name
...
def _create_import_logs(
    ...
    """
    Inserts a row into 'Custom Imported Item Logs' for every individual item in the payload.
    """
    ...
    for item in items:
        ...
        # Determine human-readable status based on UI logic
        human_status = (
            "Approved"
            if status_code == "3"
